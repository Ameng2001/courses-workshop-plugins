...
- If a project is `planning`: "Choose the pipeline for the next deliverable with `/workshop-templates:pipeline-select`"
- If no active changes: "Run `/workshop-templates:pipeline-list` to see available pipelines, or `/workshop-planner:semester-plan` to plan a semester"
