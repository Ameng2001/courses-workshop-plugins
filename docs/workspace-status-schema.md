...
4. `type: "plugin"` 和 `type: "domain"` 仅作为历史兼容元数据处理。
...
1. 若旧文件中存在 `type: "plugin"`，可在运行期将其视为 legacy project。
2. 若旧文件中字段为 `plugin` 而非 `project`，可以读取但不应再写回该旧字段。
3. 若缺失 `plan_refs`，运行期应视为：
