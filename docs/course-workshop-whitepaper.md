...
| 9 | `driving-question` | workshop-pbl | 生成 5 个候选驱动性问题，四维度打分，课程专家审核 | 高 |
| 10 | `network-map` | workshop-pbl | 从驱动问题扩展为主题网络图（子问题 → 叶节点），标注领域覆盖 | 中 |
| 11 | `inquiry-scaffold` | workshop-pbl | 将网络图拆分为 3 条递进探究线索，分配 4C 技能和学习目标 | 高 |
| 12 | `activity-design` | workshop-pbl | 为每条线索设计 3-5 个活动（编码、步骤、教师提示、资源标注） | 高 |
| 13 | `proposal-generate` | workshop-pbl | 将所有设计产出物编译为华美 PBL 五段式完整预案文档 | 中 |
| 14 | `lesson-objective` | workshop-5step | 生成可观察的教学目标（认知/技能/情感三维度），对标《指南》 | 中 |
| 15 | `lesson-scaffold` | workshop-5step | 设计五步教学环节结构（导入→探究→操作→总结→延伸）+ 时间分配 | 中 |
| 16 | `lesson-detail` | workshop-5step | 编写每个环节的教师话术、材料清单、分层指导策略 | 高 |
| 17 | `lesson-generate` | workshop-5step | 将教学目标+环节+话术编译为标准教案格式 | 低 |
...
2. 安装所需插件: workshop-core, workshop-pbl, ...
```
