...
| 技能 | 产出 | 消费（必需） | 消费（可选） |
|------|------|-------------|-------------|
| `lesson-objective` | lesson-objective.md | _(起点)_ | studio/kb/ 历年教案 |
...
| 技能 | 产出 | 消费（必需） | 消费（可选） |
|------|------|-------------|-------------|
| `semester-plan` | semester-plan.md | _(起点)_ | studio/kb/ 学期日历 |
| `month-plan` | month-plan.md | _(推荐 semester-plan.md)_ | studio/kb/ 教材 |
...
| 技能 | 产出 | 消费（必需） | 消费（可选） |
|------|------|-------------|-------------|
| `kb-import` | studio/kb/{category}/*.md | 文件路径 | — |
| `kb-index` | studio/kb/index.yaml | studio/kb/ 文档 | — |
...
Studio initialized at studio/
  ...
  studio/config.yaml   — 工作室配置（默认方法论: pbl-huamei）
...
不同幼儿园有不同的教育特色。创建自定义专家放入 `studio/agents/`，让工具理解你的园本特色：
- 蒙台梭利幼儿园 → 创建蒙台梭利教育专家
...
| `studio/kb/**/*.md` | kb-import | 知识库文档 |
| `studio/kb/index.yaml` | kb-index | 知识库索引 |
