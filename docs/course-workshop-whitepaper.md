...
| **五步法教案设计** | 一线教师设计单节课的教案，从教学目标到完整五步结构 | workshop-lesson + workshop-templates |
| **学期课程规划** | 课研主任规划学期主题日历，拆分月度和周计划 | workshop-planner + workshop-kb |
| **教学资料库管理** | 导入和查询本校历年教案、教材、教研记录 | workshop-kb |
| **教学方法论切换** | 查看和选择不同教学方法论管线（PBL、五步法等） | workshop-templates |
...
| **workshop-templates** | 方法论管线 | 4 | 列出可用教学方法论管线、选择并激活管线，并兼容旧命令别名 |
...
workshop-templates   零依赖（独立运行）
workshop-kb          依赖 workshop-core（需要工作空间）
workshop-designer    依赖 workshop-core + workshop-templates（需要工作空间和方法论管线）
workshop-lesson      依赖 workshop-core + workshop-templates（需要工作空间和方法论管线）
workshop-planner     依赖 workshop-core + workshop-templates（需要工作空间和方法论管线）
...
```
    ┌──────────────────┐   ┌──────────────────┐
    │  workshop-core   │   │workshop-templates │ ← 基础层
    └────────┬─────────┘   └────────┬─────────┘
             │                      │
    ┌────────┼──────────────────────┼────────────┐
    │        │                      │            │
...
| 4 | `pipeline-list` | workshop-templates | 列出所有可用教学方法论管线及其阶段信息 | 低 |
| 5 | `pipeline-select` | workshop-templates | 选择并激活教学方法论管线到当前工作区 | 低 |
...
```
  ...
  lesson-objective.md  lesson-scaffold.md              lesson-plan.md
       ↑                  ↑
  (workshop-kb        (workshop-templates
   历年教案参考)        five-step 模板)
```
...
最小可用组合：
- **PBL**：workshop-core + workshop-templates + workshop-designer（3 个插件，10 个技能）
- **五步法**：workshop-core + workshop-templates + workshop-lesson（3 个插件，9 个技能）

推荐完整安装 **全部 9 个插件**（27 个技能），获得多方法论设计、课程规划、知识库和质量保障的完整能力。
...
Next steps:
  /workshop-templates:pipeline-list — 查看可用教学方法论管线
...
使用 `/workshop-templates:pipeline-list` 查看可用管线。PBL 适合跨领域整合探究项目（月度粒度），五步法适合单节课的结构化教学（课时粒度）。两种方法论可以在同一学期中混合使用。
