...
使用 Course Workshop 的方式：
  - 使用 /workshop-5step:lesson 设计五步法教案
...
| 场景 | 说明 | 推荐使用的插件 |
|------|------|-------------|
| **月度 PBL 预案设计**（最核心场景） | 从月度主题出发，完成一份符合华美 PBL 五段式的完整预案 | 全部 11 个插件 |
| **新主题快速试探** | 课研团队考虑某个主题是否适合做 PBL 项目，需要快速评估主题的教育价值和探究潜力 | workshop-insight（仅 `theme-analysis`） |
| **课研团队培训** | 新入职课研人员学习 PBL 预案设计流程，通过工具内置的方法论规则理解"好的预案长什么样" | 全部 11 个插件（作为教学工具） |
...
| **五步法教案设计** | 一线教师设计单节课的教案，从教学目标到完整五步结构 | workshop-5step + workshop-pipelines |
| **主题式课程包交付** | 面向客户的月主题课程包，输出主题解读、主题网络、月矩阵、周编排及多活动类型稿件 | workshop-pipelines + workshop-insight + workshop-planner + workshop-5step + workshop-activity + workshop-format |
...
Course Workshop 由 11 个插件组成，各司其职：

| 插件 | 定位 | 技能数 | 一句话说明 |
|------|------|--------|-----------|
| **workshop-core** | 工作空间管理 | 7 | 初始化、配置、onboarding、状态、计划关联、审批与归档 |
| **workshop-pipelines** | 方法论管线 | 2 | 列出可用教学方法论管线、选择并激活管线 |
| **workshop-insight** | 前期分析 | 5 | 主题分析、主题解读、主题网络图、前期经验评估、4C 能力映射 |
| **workshop-pbl** | PBL 课程设计 | 5 | 驱动问题、网络图、线索拆分、活动设计、预案生成 |
| **workshop-5step** | 五步法教案 | 4 | 教学目标、环节设计、详细话术、教案编译 |
| **workshop-activity** | 主题活动稿 | 4 | 区域活动、户外游戏、生活渗透、家园互动 |
...
| **workshop-format** | 导出与格式化 | 2 | 客户版式标准化、导出包准备 |
...
workshop-format      依赖 workshop-core + workshop-pipelines（需要工作空间和导出格式层）
workshop-kb          依赖 workshop-core（需要工作空间）
workshop-pbl    依赖 workshop-core + workshop-pipelines（需要工作空间和方法论管线）
workshop-5step      依赖 workshop-core + workshop-pipelines（需要工作空间和方法论管线）
workshop-activity   依赖 workshop-core + workshop-pipelines（需要工作空间和方法论管线）
...
```
    ...
    ▼        ▼                      ▼            ▼
  ┌────────────────┐  ┌───────────────┐  ┌──────────────┐
  │workshop-pbl│  │workshop-5step│  │workshop-planner│ ← 依赖 core + pipelines
  └────────────────┘  └───────────────┘  └──────────────┘
  ┌────────────────┐  ┌────────────────┐
...
**实际使用中的推荐组合**：大多数情况下，建议安装全部 11 个插件以获得完整体验。如果只需要部分功能，参考第 4 节的场景推荐。
...
| 2 | `config` | workshop-core | 查看或修改运行时默认配置、模型和发布参数 | 低 |
| 3 | `onboarding` | workshop-core | 检查当前工作台 readiness，并推荐下一步操作 | 低 |
| 4 | `status` | workshop-core | 查看当前工作空间的设计进度、HIL 状态和技能完成度 | 低 |
| 5 | `link-plan` | workshop-core | 将课程项目与周/月/学期规划记录建立引用关系 | 低 |
| 6 | `approve` | workshop-core | 将项目推进到最终 approval-gate 并记录审批信息 | 低 |
| 7 | `promote` | workshop-core | 将完成项目归档并发布为课程 release bundle | 低 |
| 8 | `pipeline-list` | workshop-pipelines | 列出所有可用教学方法论管线及其阶段信息 | 低 |
| 9 | `pipeline-select` | workshop-pipelines | 选择并激活教学方法论管线到当前工作区 | 低 |
| 10 | `theme-analysis` | workshop-insight | 分析月度主题的教育价值、《指南》领域覆盖、课程标准对齐 | 中 |
| 11 | `theme-narrative` | workshop-insight | 生成客户交付口径的“主题解读”文稿 | 中 |
| 12 | `theme-network` | workshop-insight | 生成“核心主题→子主题→领域目标簇”的主题网络图 | 中 |
| 13 | `prior-knowledge` | workshop-insight | 评估幼儿的前期经验（认知/技能/情感三维度），含年龄段差异对照 | 中 |
| 14 | `competency-mapping` | workshop-insight | 4C 能力映射 + 学习目标生成，含误分类检测和前期经验桥接分析 | 高 |
| 15 | `driving-question` | workshop-pbl | 生成 5 个候选驱动性问题，四维度打分，课程专家审核 | 高 |
| 16 | `network-map` | workshop-pbl | 从驱动问题扩展为主题网络图（子问题 → 叶节点），标注领域覆盖 | 中 |
| 17 | `inquiry-scaffold` | workshop-pbl | 将网络图拆分为 3 条递进探究线索，分配 4C 技能和学习目标 | 高 |
| 18 | `activity-design` | workshop-pbl | 为每条线索设计 3-5 个活动（编码、步骤、教师提示、资源标注） | 高 |
| 19 | `proposal-generate` | workshop-pbl | 将所有设计产出物编译为华美 PBL 五段式完整预案文档 | 中 |
| 20 | `lesson-objective` | workshop-5step | 生成可观察的教学目标（认知/技能/情感三维度），对标《指南》 | 中 |
| 21 | `lesson-scaffold` | workshop-5step | 设计五步教学环节结构，并嵌入重点/难点与观察支持要点 | 中 |
| 22 | `lesson-detail` | workshop-5step | 编写每个环节的教师话术、材料清单、分层指导策略 | 高 |
| 23 | `lesson-generate` | workshop-5step | 将教学目标+环节+话术编译为客户交付口径的主题教学活动稿 | 中 |
| 24 | `region-activity` | workshop-activity | 生成区域活动稿，覆盖预期经验、材料投放和指导策略 | 中 |
| 25 | `outdoor-game` | workshop-activity | 生成户外游戏稿，覆盖玩法、安全提示与场地组织 | 中 |
| 26 | `life-routine` | workshop-activity | 生成生活渗透稿，覆盖教师提示、介入时机与日常实施要点 | 低 |
| 27 | `home-school` | workshop-activity | 生成家园互动稿，覆盖家庭任务、回收方式与教师发送提示 | 低 |
| 28 | `semester-plan` | workshop-planner | 创建学期主题日历，月度主题分配，领域均衡检查 | 高 |
| 29 | `month-plan` | workshop-planner | 生成月度主题活动矩阵、周次递进与材料预览 | 高 |
| 30 | `week-plan` | workshop-planner | 生成 15-17 项周活动编排、材料清单和教师备忘 | 高 |
| 31 | `standards-check` | workshop-quality | 8 项规则自动检查（课标覆盖、动词适切、4C 准确性、时长等） | 中 |
| 32 | `proposal-review` | workshop-quality | 3 位专家 Agent 联合评审，9 维度评分，Top 5 改进建议 | 高 |
| 33 | `resource-planner` | workshop-resource | 按 4 类（PBL Box/足迹袋/自备/多媒体）归类资源，生成采购汇总 | 中 |
| 34 | `resource-check` | workshop-resource | 5 项资源校验（覆盖度/数量/目录匹配/重复检测/分类正确性） | 中 |
| 35 | `kb-import` | workshop-kb | 导入园本教学资料（教材、教案、教研记录），结构化存储 | 中 |
| 36 | `kb-index` | workshop-kb | 构建/刷新知识库索引，支持快速检索 | 中 |
| 37 | `kb-query` | workshop-kb | 按主题、年龄段、领域等条件检索知识库 | 低 |
| 38 | `format-lesson` | workshop-format | 将教学活动稿整理为客户版式友好的导出源文件 | 中 |
| 39 | `export-bundle` | workshop-format | 生成 word-ready / pdf-ready / remote-ready 导出包与 manifest | 中 |

### 5.4 四条设计流水线

Course Workshop 提供三条独立的设计流水线，分别对应不同的教学方法论和粒度：

#### 流水线 A：PBL 预案设计（`/workshop-pbl:design`）
...
#### 流水线 B：五步法教案设计（`/workshop-5step:lesson`）

```
  [workshop-5step 教案流水线]
...
#### 流水线 D：主题式课程包交付（`/workshop-pipelines:pipeline-select thematic-curriculum`）

```
  [workshop-insight 主题 framing]
  theme-analysis → theme-narrative → theme-network
                                       │
                                       ▼
  [workshop-planner 主题编排]
  ...
  workshop-activity: region-activity / outdoor-game / life-routine / home-school
             │
             ▼
  [导出层]
  workshop-format: format-lesson → export-bundle
```

四条流水线可独立使用，也可串联：先用流水线 C 做规划，再进入流水线 D 产出客户主题课程包；或直接进入流水线 A、B 处理 PBL 与单次教学活动场景。
...
│  workshop-5step     ← 五步法教案设计               │
│  workshop-activity  ← 主题式课程活动稿生成          │
├─────────────────────────────────────────────────┤
│                   扩展（Add-ons）                  │
│                                                   │
...
│  workshop-format     ← 导出与格式化（推荐但可选）     │
└─────────────────────────────────────────────────┘
```

最小可用组合：
- **PBL**：workshop-core + workshop-pipelines + workshop-pbl（3 个插件，14 个技能）
- **五步法**：workshop-core + workshop-pipelines + workshop-5step（3 个插件，13 个技能）
- **主题式课程包**：workshop-core + workshop-pipelines + workshop-insight + workshop-planner + workshop-5step + workshop-activity + workshop-format（7 个插件，27 个技能）

推荐完整安装 **全部 11 个插件**（39 个技能），获得多方法论设计、课程规划、主题式课程包交付、知识库和质量保障的完整能力。
...
| 平台 | 加载命令 |
|------|---------|
| Claude Code | `claude --plugin-dir ./workshop-core --plugin-dir ./workshop-pbl ...` |
...
2. 安装所需插件: workshop-core, workshop-pbl, workshop-5step, workshop-activity, ...
```
...
Next steps:
  ...
  /workshop-pbl:design <主题>  — 设计 PBL 预案
  /workshop-5step:lesson <课题>    — 设计五步法教案
...
先运行 `workshop-insight`（theme-analysis + prior-knowledge + competency-mapping），再运行 `workshop-pbl`。前期分析的产出让设计更有依据。
...
| designing | /workshop-pbl:design | 正在设计预案 |
